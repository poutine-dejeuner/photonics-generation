...
# SMALL MODEL VERSION - Using reduced model size to avoid OOM
...
# Calculate total number of jobs (1 run per n_samples value)
n=1
total_jobs=$((${#n_samples_values[@]} * n))
max_array_index=$((total_jobs - 1))

echo "Submitting array job for train3.py with small model..."
...
#SBATCH --job-name=train3_small_array
...
# Calculate which n_samples value to use based on SLURM_ARRAY_TASK_ID
n_samples_index=\$SLURM_ARRAY_TASK_ID
...
python train3.py training.n_samples=\$n_samples model=unet_small training.batch_size=4
EOF
