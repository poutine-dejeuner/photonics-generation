...
def train(data: np.ndarray, cfg, checkpoint_path: str, savedir: str, run=None):
    ...
    total_params = count_parameters(wgan.generator) + count_parameters(wgan.critic)
    assert (total_params - N) < 0.14 * N
    
    optimizer_G = optim.Adam(wgan.generator.parameters(), lr=lr_g, betas=(0.5, 0.9))
    optimizer_C = optim.Adam(wgan.critic.parameters(), lr=lr_c, betas=(0.5, 0.9))
